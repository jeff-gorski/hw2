...
# Generated using rails generate model __________
# Went into db to add the appropriate columns
# Once complete, migrated using rails db:migrate
...
jeff_gorski = Agent.create(name: "Jeff Gorski")

christian_bale = Actor.create(name: "Christian Bale", agent: jeff_gorski)
...
movies.each do |movie|
  puts "#{movie.title.ljust(23)}#{movie.year_released}#{movie.rated.to_s.rjust(16)}  #{movie.studio.name}"
end
...
movies.each do |movie|
  roles = movie.roles.includes(:actor)
  roles.each do |role|
    puts "#{movie.title.ljust(23)}#{role.actor.name.ljust(22)}#{role.character_name}"
  end
end
